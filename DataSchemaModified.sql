...
CREATE TABLE user (
    ...
    CHECK (regexp_like(name, '^[A-ZА-Я][a-zа-я\- ]{1,49}$')),
...
CREATE TABLE sleep_phase (
    ...
    phase_duration INTERVAL HOUR TO MINUTE CHECK (phase_duration >= INTERVAL '1 minute' AND phase_duration <= INTERVAL '8 hours'),
...
CREATE TABLE recommendation (
    ...
    user_id INTEGER NOT NULL REFERENCES user(user_id),
    recommendation_text VARCHAR(500) NOT NULL CHECK (regexp_like(recommendation_text, '^[A-Za-zА-Яа-я0-9,.\-\s]{1,500}$')),
...
CREATE TABLE checklist (
    ...
    completion_percentage FLOAT CHECK (completion_percentage >= 0.0 AND completion_percentage <= 100.0)
);
